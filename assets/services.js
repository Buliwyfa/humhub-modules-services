function loadServicesMessage(messageId) {
    ...
    if ($("#mail_message_details").length) {
        ...
        $.ajax({
            'type': 'GET',
            'url': services_loadMessageUrl.replace('-messageId-', messageId),
            'cache': false,
        ...
        // Somewhere outside
        window.location.replace(services_viewMessageUrl.replace('-messageId-', messageId));
    }
